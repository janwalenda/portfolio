...
Instead of a huge array internally _trying_ to manage various entry points, we created separate config files (e.g., `rollup.config.purchase.mjs`). This brings two massive benefits:
...
We used the **`@rollup/plugin-commonjs`** (for old NPM modules) and the **`@rollup/plugin-node-resolve`** (so Rollup knows where to find modules). But the key was the consistent **module-based writing** of the legacy code. Each new bundle had a clear, small entry point that _explicitly_ imported only the functions it required.

> **My Pro-Tip:** If you're working with Rollup and encountering unwanted code, always start with the **`external`** array in your config. Define everything there that should _not_ be in the bundle (e.g., `react`, `jquery`). This forces you to define dependencies cleanly.
